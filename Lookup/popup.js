...
function init() {
	...
	chrome.storage.sync.get(saves, function(result) {
		...
		for (
			var i = 0;
			i < document.getElementById(pre_html_id + "grounding").options.length;
			i++
		) {
			if (
				document.getElementById(pre_html_id + "grounding").options[i].value ==
				grounding
			) {
				document.getElementById(pre_html_id + "grounding").options[
					i
				].selected = true;
				break;
			}
		...
		for (
			i = 0;
			i <
			document.getElementById(pre_html_id + "input_language").options.length;
			i++
		) {
			if (
				document.getElementById(pre_html_id + "input_language").options[i]
					.value == input_language
			) {
				document.getElementById(pre_html_id + "input_language").options[
					i
				].selected = true;
				break;
			}
		...
		if (switcher_grounding === true)
			document.getElementById(pre_html_id + "grounding").style.display =
				"inline";
		else
...
window.addEventListener("load", function(evt) {
	...
	document.getElementById(pre_html_id + "options_page").innerHTML =
		'<a href="' +
		chrome.extension.getURL("options.html") +
		'" target="_blank">Extension Options</a>';

	// Loading the groundings
	for (var current in engine) {
		document.getElementById(pre_html_id + "grounding").innerHTML +=
			'<option value="' +
			current +
			'" id="' +
			current +
			'">' +
			engine[current].info[1] +
			"</option>";
	}
	...
	document
		.getElementById(pre_html_id + "grounding")
	...
	document
		.getElementById(pre_html_id + "input_language")
		.addEventListener("change", function() {
			var input_language = document.getElementById(
				pre_html_id + "input_language"
			).value;
	...
	document
		.getElementById(pre_html_id + "search")
	...
	document.onkeydown = function(event) {
		...
		if (code == 40) {
			...
			if (
				history == last_queries.length &&
				document.getElementById(pre_html_id + "query").value !== ""
			) {
