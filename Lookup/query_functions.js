...
function assemble_search_engines(
	language,
	grounding,
	input_language,
	switcher_grounding,
	...
	if (language == "en")
		...
		result = [
			["duden", engine.duden.url(language, input_language), "", ""],
			["wikipedia", engine.wikipedia.url(language, input_language), "", ""],
			["dict", engine.dict.url(language, input_language), "", ""]
		];
	else
	...
	if (switcher_grounding === true) {
		...
		result.unshift([
			grounding,
			engine[grounding].url(language, input_language),
			"",
			""
		]);
	}
	...
	if (switcher_ranked_search === false)
		result = [
			[grounding, engine[grounding].url(language, input_language), "", ""]
		];
...
var lookup = {
	...
	process: function() {
		...
		for (var i = 0; i < search_engines.length; i++) {
			...
			if (search_engines[i][2] < 4) return;
				...
				search_engines[i][2] == "none" &&
				search_engines[i][3].length < 4
			)
				search_engines[i][3] = "none";
			else if (
				search_engines[i][2] != "none" &&
				search_engines[i][3].length < 4
			) {
				// Invoke the various search engines for further processing of the html-code
				search_engines[i][3] = engine[search_engines[i][0]].innerText(
					search_engines[i][2]
				);
			}
			...
			if (
				document.getElementById(pre_html_id + "output").style.display ==
				"inline"
			)
				return;
			...
			if (search_engines[i][3] == "none" && i == search_engines.length - 1) {
				...
				document.getElementById(pre_html_id + "noresult").innerHTML =
					'<p>No Match - <a href="https://www.google.de/search?q=' +
					encodeURIComponent(query) +
					'" target="_blank">Google for "' +
					tmp
						.replace(/"/g, '"')
						.replace("&", "&amp;")
						.replace("<", "&lt;")
				...
				document.getElementById(pre_html_id + "noresult").style.display =
					"inline";
				...
				if (strip_html(search_engines[i][3]).length >= max_output_length) {
					search_engines[i][3] = search_engines[i][3].slice(
						0,
						max_output_length
					);
					search_engines[i][3] =
						search_engines[i][3].slice(
							0,
							search_engines[i][3].lastIndexOf(" ")
						) + "...";
				}

				// Filling the various html divisions
				document.getElementById(pre_html_id + "output").innerHTML =
					"<p></p>" + search_engines[i][3];
				document.getElementById(pre_html_id + "source").innerHTML =
					'<p><span class="tab"></span><i><a href="' +
					search_engines[i][1] +
					encodeURIComponent(query) +
					'" target="_blank">' +
					search_engines[i][1] +
				...
				document.getElementById(pre_html_id + "output").style.display =
					"inline";
				document.getElementById(pre_html_id + "source").style.display =
					"inline";
			}
		}
	},
	...
	init: function() {
		...
		for (var i = 0; i < last_queries.length; i++) {
			if (last_queries[i] !== null && last_queries[i].indexOf(query) != -1)
				last_queries.splice(i, 1);
		}
		last_queries.push(query);
		// Keeping the maximum length of last_queries below max_last_queries
		if (last_queries.length > max_last_queries)
			last_queries = last_queries.slice(last_queries.length - max_last_queries);
		...
		document.getElementById(pre_html_id + "loading").innerHTML =
			"<p>Searching in " +
			search_engines
